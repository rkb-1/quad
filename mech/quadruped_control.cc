  ...
  namespace mech
  {
    ...
    public:
      ...
      void UpdateTerrain()
      {
        ...
        // controlling legs.
        if (stance_A.empty() || old_control_log_->legs_R.empty())
        {
          return;
        }
        ...
        const double alpha = (std::pow(0.5, config_.period_s / config_.terrain_filter_s));
        robot.terrain_rad[0] = (alpha * robot.terrain_rad[0] + (1.0 - alpha) * std::atan(plane.a));
        robot.terrain_rad[1] = (alpha * robot.terrain_rad[1] + (1.0 - alpha) * std::atan(plane.b));

        robot.tf_TA.so3() = Sophus::SO3d(
            (base::Quaternion::FromAxisAngle(
                 robot.terrain_rad[0], 0, 1, 0) *
             base::Quaternion::FromAxisAngle(
                 -robot.terrain_rad[1], 1, 0, 0))
                .eigen());
      }
      ...
      void RunControl()
      {
        ...
        switch (status_.mode)
        {
          case QM::kConfiguring:  { DoControl_Configuring();  break;}
          case QM::kStopped:      { DoControl_Stopped();  break;}
          case QM::kFault:        { DoControl_Fault();  break;}
          case QM::kZeroVelocity: { DoControl_ZeroVelocity(); break;}
          case QM::kJoint:        { DoControl_Joint();  break;}
          case QM::kLeg:          { DoControl_Leg();  break;}
          case QM::kStandUp:      { DoControl_StandUp();  break;}
          case QM::kRest:         { DoControl_Rest(); break;}
          case QM::kJump:         { DoControl_Jump(); break;}
          case QM::kWalk:         { DoControl_Walk(); break;}
          case QM::kBackflip:     { DoControl_Backflip(); break;}
          case QM::kJointTraj:    { DoControl_Trajectory(y_vec[temp_time], yd_vec[temp_time], Tau_vec[temp_time]);  break;}
          case QM::kNumModes:     { mjlib::base::AssertNotReached();}
        }
      }
          ...
          case M::kDone: 
                        {
                          ...
                          //     std::cout<< "pose from config file back to position: " << config_.leg_onemove.pose_R[0] <<std::endl;
                          //     leg_R.velocity = base::Point3D();
                          //     leg_R.stance = 1.0;
                          //     //legs_R.push_back(leg_R);
                          //   }
      ...
      void DoControl_StandUp()
      {
        ...
        const double elapsed_s = base::ConvertDurationToSeconds(Now() - status_.mode_start);
        if (status_.state.stand_up.mode != M::kDone && elapsed_s > config_.stand_up.timeout_s)
        {
          Fault("timeout");
          return;
        }
        //std::cout << "debug status standup mode : " << status_.state.stand_up.mode << std::endl;
      ...
      bool CheckPrepositioning() const
      {
        ...
        for (const auto &leg : context_->legs)
        {
          ...
          if (!check(leg.config.ik.shoulder.id, leg.resolved_stand_up_joints.shoulder_deg))
          {
            return false;
          }
          if (!check(leg.config.ik.femur.id, leg.resolved_stand_up_joints.femur_deg))
          {
            return false;
          }
          if (!check(leg.config.ik.tibia.id, leg.resolved_stand_up_joints.tibia_deg))
          {
            return false;
          }
        }
      ...
      void DoControl_StandUp_Prepositioning()
      {
        ...
        //std::cout << context_->legs << std::endl;
        for (const auto &leg : context_->legs)
        {
          QC::Joint joint;
          joint.power = true;
          ...
          switch (status_.state.stand_up.prepositioning_stage)
          {
            case 0:
            {
              ...
              // joints unchanged.
              //std::cout<<"config file: "<<leg.config.ik.shoulder.id <<"degrees: "<<leg.shoulder_clearance_deg <<std::endl;
        ...
        for (const auto &joint : joints)
        {
          ...
          if (!!joint.stop_angle_deg && std::abs(joint.stop_angle_deg.value_or(0.0) - joint_state.angle_deg) > config_.stand_up.tolerance_deg)
          {
            all_done = false;
          }
        }
        if (all_done)
        {
          status_.state.stand_up.prepositioning_stage = std::min(status_.state.stand_up.prepositioning_stage + 1, 2);
        }
      ...
      void DoControl_StandUp_Standing()
      {
        ...
        //std::cout<<"Legs_R " <<legs_R[0]<<std::endl;
        if (legs_R.empty())
        {
          for (const auto &leg : context_->legs)
          {
      ...
      {
        if (status_.mode != QM::kRest)
        {
          return false;
        }
        if (control_log_->legs_R.empty())
        {
          return false;
        }
        ...
        {
          if (leg_R.velocity != base::Point3D())
          {
            return false;
          }
        }
      ...
      void DoControl_Rest()
      {
        ...
        QuadrupedContext::MoveOptions move_options;
        move_options.override_acceleration = config_.stand_up.acceleration;
        status_.state.rest.done = context_->MoveLegsFixedSpeedZ(all_leg_ids_,
                                                                &legs_R,
      ...
      bool SetLandingParameters(std::vector<QC::Leg> *legs_R)
      {
        ...
        const double command_velocity =
            (average_height > config_.jump.lower_height) ? limited_velocity : 0.0;
        ...
        // the ground).
        const bool done = ((average_height < config_.jump.lower_height) ||
      ...
      void BackflipUpdateLegs(std::vector<QC::Leg> *legs_R, double acceleration)
      {
        auto &bs = status_.state.backflip;
          ...
          else
          {
            ...
            leg_R.force_N =
                ...
                (leg_R.stance / stance_legs) *
                    base::Point3D(
                        0, 0, config_.mass_kg * (base::kGravity + acceleration));
          }
        }
      ...
      void ControlLegs_R(std::vector<QC::Leg> legs_R, const base::KinematicRelation &desired_RB)
      {
        control_log_->desired_RB = desired_RB;
        control_log_->legs_R = std::move(legs_R);
        std::sort(control_log_->legs_R.begin(), control_log_->legs_R.end(), [](const auto &lhs, const auto &rhs)
        ...
        // filter.
        {
          ...
          const base::Point3D delta = desired_RB.pose.translation() - frame_RB.pose.translation();
          frame_RB.pose.translation() += config_.rb_filter_constant_Hz * config_.period_s * delta;
          frame_RB.pose.so3() = Sophus::SO3d(frame_RB.pose.so3().unit_quaternion().slerp(
      ...
      void ControlLegs_B(std::vector<QC::Leg> legs_B)
      {
        ...
        std::sort(control_log_->legs_B.begin(), control_log_->legs_B.end(), [](const auto &lhs, const auto &rhs)
                  { return lhs.leg_id < rhs.leg_id; });

        // Apply Z bounds.
        for (auto &leg_B : control_log_->legs_B)
        {
          leg_B.position.z() = std::max(config_.bounds.min_z_B, std::min(config_.bounds.max_z_B, leg_B.position.z()));
        }
        ...
        for (const auto &leg_B : control_log_->legs_B)
        {
          ...
          else
          {
            ...
            leg_pd.gravity_N = stance_fraction * base::kGravity * config_.mass_kg * g_B;

            leg_pd.accel_N = (leg_B.acceleration) * base::Interpolate(config_.leg_mass_kg, stance_fraction * config_.mass_kg, leg_B.stance);
            leg_pd.err_m = leg_state_B.position - leg_B.position;
            leg_pd.p_N = -1 * (leg_pd.err_m.array() * leg_B.kp_N_m.array()).matrix();
            leg_pd.err_m_s = leg_state_B.velocity - leg_B.velocity;
            leg_pd.d_N = -1 * (leg_pd.err_m_s.array() * leg_B.kd_N_m_s.array()).matrix();
            leg_pd.total_N = leg_pd.cmd_N + leg_pd.gravity_N + leg_pd.accel_N + leg_pd.p_N + leg_pd.d_N;
            ...
            else
            {
              for (const auto &joint_angle : *result)
              {
                ...
                out_joint.kp_scale = leg_B.kp_scale ? get_pd_scale(*leg_B.kp_scale) : std::optional<double>();
                out_joint.kd_scale = leg_B.kd_scale ? get_pd_scale(*leg_B.kd_scale) : std::optional<double>();
                out_joints.push_back(out_joint);
              }
            }
          }
      ...
      void EmitControl()
      {
        ...
        for (const auto &joint : control_log_->joints)
        {
          ...
          std::optional<double> max_torque_Nm =
              (parameters_.max_torque_Nm >= 0.0 || !!joint.max_torque_Nm) ? std::min(parameters_.max_torque_Nm < 0.0 ? kInf : parameters_.max_torque_Nm,
          ...
          if (mode == moteus::Mode::kPosition)
          {
            ...
            if (joint.angle_deg != 0.0)
            {
              values.resize(1);
            }
            if (joint.velocity_dps != 0.0)
            {
              values.resize(2);
            }
      ...
      void EmitRezero()
      {
        ...
        for (const auto &joint : config_.joints)
        {
          ...
          request.request.WriteSingle(moteus::kRezero, static_cast<float>(joint.rezero_pos_deg / 360.0));
        }
      }
      ...
      boost::signals2::signal<void(const ReportedServoConfig *)> servo_config_signal_;
